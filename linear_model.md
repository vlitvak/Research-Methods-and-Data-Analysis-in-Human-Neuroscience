...
The answer is that the statistical analysis has not accounted for the covariance in the data,
which has the structure
```
S = kron(eye(10),ones(2));
```
If we know this covariance, then it can be included within the various matrix operations of the statistical analysis
to make it correct.
You don't need to know or understand the details, but this is done as follows:
WORK IN PROGRESS
```
beta2 = (X2' * S * X2) \ X2'* S *y2
nu2 = size(X2,1) - rank(X2);
r2  = y2 - X2*beta2;
v2  = (r2' * S * r2)/nu2
t2 = (c * beta2) / sqrt(v2 * c * inv(X2' * S * X2) * c')
p2 = 1-spm_Tcdf(t2, nu2)
```
